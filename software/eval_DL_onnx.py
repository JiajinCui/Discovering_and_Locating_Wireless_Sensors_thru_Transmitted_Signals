...
def main():
    ...
    with open(iq_fp) as binfile:
        all_samps = np.fromfile(binfile, dtype=np.complex64, count=1024*1024 *2, offset=0)
    ...
    a, b = np.meshgrid(np.linspace(0,1024*1024,1024*2),np.linspace(0,1024,1024))
    fig, ax = plt.subplots(1,2,sharey=True,figsize=(9,4))
    all_samps = np.reshape(all_samps, -1)
    spectrum,_,_,_ = plt.specgram(all_samps,noverlap=int(0),NFFT=int(1024), Fs = 25)
    ...
    cs = ax[1].contourf(a, b, y_pred.T, levels=[0.5, 1.5, 2.5, 3.5, 4.5, 5.5, 6.5], cmap='Dark2') # change for unknown

    protocol_dict = {0:"empty", 1:"Wi-Fi", 2:"LTE", 3:"Zigbee", 4:"LoRa", 5:"BLE", 6:"Z-Wave"}
    ...
    # Loop over ranked elements
    for rank, (element, frequency) in enumerate(zip(sorted_unique, sorted_counts), start=1):
        print(f"Rank {rank}: {protocol_dict[element]} appears {frequency} times.")
    ...
    #plt.figlegend(proxy,["Z-Wave"])

    plt.figlegend(proxy,["Wi-Fi", "LTE", "Zigbee", "LoRa", "BLE", "Z-Wave"]   )       # change for unknown
    plt.tight_layout()
    plt.show()
